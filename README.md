...
一个用于扫描和分析WiFi信道使用情况的Python工具，支持Windows、macOS和Linux系统。

## 🎉 最新更新 (v4.0 - 2026-03-28)

### 🚀 重大功能升级 - 模块化重构与跨平台优化

#### 1. **代码模块化重构**
- ✅ **独立模块设计**: 将核心功能拆分为独立的Python模块
- ✅ **跨平台工具类**: 创建统一的跨平台工具类，确保代码一致性
- ✅ **接口标准化**: 所有模块使用统一的接口和错误处理机制
- ✅ **依赖管理**: 优化模块间的依赖关系，提高代码可维护性

#### 2. **硬件信息检测系统**
- ✅ **跨平台硬件检测**: 支持Windows、macOS、Linux系统的硬件信息获取
- ✅ **CPU信息检测**: 自动识别CPU型号、核心数、频率等信息
- ✅ **显卡信息检测**: 检测显卡型号和显存容量
- ✅ **内存信息检测**: 获取总内存、可用内存和使用率
- ✅ **性能评分系统**: 基于硬件规格计算综合性能评分（0-100分）

#### 3. **网络速度测试系统**
- ✅ **双模式测速**: 支持系统命令模式和下载文件模式
- ✅ **全面指标检测**: 下载速度、上传速度、延迟、抖动
- ✅ **智能服务器选择**: 多测试服务器自动回退机制
- ✅ **实时进度显示**: 下载测速时显示实时进度和速度
- ✅ **跨平台兼容**: 适配不同操作系统的网络命令

#### 4. **视频分辨率智能推荐系统**
- ✅ **多平台支持**: 抖音、虎牙、优酷、腾讯视频、爱奇艺、哔哩哔哩
- ✅ **智能推荐算法**: 根据网络速度和硬件性能推荐最优分辨率
- ✅ **硬件性能考虑**: 结合CPU、显卡、内存性能调整推荐策略
- ✅ **安全带宽预留**: 保留20%带宽应对网络波动
- ✅ **分辨率级别**: 4K、1080P、720P、480P、360P完整支持

#### 5. **跨平台工具类集成**
- ✅ **统一命令执行**: 智能编码检测，支持UTF-8和系统编码
- ✅ **平台信息获取**: 自动检测操作系统类型和版本
- ✅ **乱码检测**: 自动检测和避免中文乱码问题
- ✅ **安全打印**: 确保中文在不同平台正确显示
- ✅ **错误处理**: 完善的异常捕获和回退机制
...
- [HARDWARE_INFO.md](HARDWARE_INFO.md) - 硬件信息检测器详细文档
- [NETWORK_SPEED_TESTER.md](NETWORK_SPEED_TESTER.md) - 网络速度测试器详细文档  
- [VIDEO_RESOLUTION_RECOMMENDER.md](VIDEO_RESOLUTION_RECOMMENDER.md) - 视频分辨率推荐器详细文档

## 📖 历史版本

### v3.0 (2026-03-27) - 无线网卡信息增强

#### 1. **无线网卡品牌型号检测系统**
- ✅ **智能品牌识别**：自动检测USB无线网卡和笔记本内置网卡的品牌型号
- ✅ **腾达网卡支持**：支持腾达U12、U9、W311U等主流USB网卡型号检测
- ✅ **笔记本品牌覆盖**：支持联想、华硕、戴尔、惠普等20+主流笔记本品牌
- ✅ **芯片到品牌映射**：基于Realtek等芯片信息智能推断具体品牌型号

#### 2. **网卡带宽信息集成**
- ✅ **最大带宽检测**：自动检测网卡的理论最大带宽（Mbps）
- ✅ **WiFi标准判断**：基于带宽和网卡描述智能判断WiFi技术标准
- ✅ **友好显示格式**：同时提供技术数据和易于理解的显示格式
- ✅ **完整信息保存**：在JSON文件中保存完整的网卡性能信息

#### 3. **中文乱码全面修复**
- ✅ **智能编码检测**：支持UTF-8、GBK、CP936等多种编码格式
- ✅ **乱码字符检测**：自动检测和修复中文乱码问题
- ✅ **跨平台兼容**：确保Windows和macOS系统下中文显示正常
- ✅ **JSON文件优化**：所有中文字符在JSON文件中正确显示
...
- ✅ **网卡性能数据**：新增网卡品牌型号、最大带宽、WiFi标准等信息
...
```json
{
  ...
  "network_card": {
    "网卡完整描述": "Realtek 8811CU Wireless LAN 802.11ac USB NIC",
    "max_bandwidth_mbps": 433,
    "最大支持带宽": "433M",
    "网卡型号": "腾达U12 (基于Realtek 8811CU)",
    "WiFi标准": "WiFi 5 (802.11ac)"
  },
  "location": {
    "country": "China",
    "region": "XX省",
...
### v3.0 (2026-03-27) - **大版本升级**
- 🚀 **无线网卡品牌型号检测系统**：智能检测USB无线网卡和笔记本内置网卡品牌型号
- 📡 **网卡带宽信息集成**：自动检测网卡最大带宽和WiFi技术标准
- 🔧 **中文乱码全面修复**：彻底解决Windows系统中文显示乱码问题
- 💻 **笔记本品牌全面覆盖**：支持20+主流笔记本品牌型号检测

#### 新增功能
- ✅ **腾达网卡支持**：支持腾达U12、U9、W311U等主流USB网卡型号检测
- ✅ **笔记本品牌检测**：联想、华硕、戴尔、惠普等20+主流品牌
- ✅ **带宽标准判断**：基于带宽智能判断WiFi 4/5/6技术标准
- ✅ **友好显示格式**：同时提供技术数据和易于理解的显示格式

#### 技术优化
- ✅ **智能编码检测**：支持UTF-8、GBK、CP936等多种编码格式
- ✅ **乱码字符修复**：自动检测和修复中文乱码问题
- ✅ **跨平台兼容**：确保Windows和macOS系统下中文显示正常
- ✅ **JSON数据增强**：新增网卡品牌型号、最大带宽、WiFi标准等信息

#### 修复问题
- 🐛 **网卡型号显示**：修复网卡型号字段显示芯片信息而非品牌型号的问题
- 🔧 **代码缩进修复**：修复品牌型号检测逻辑的缩进错误
- 📊 **数据一致性**：确保控制面板与JSON文件显示的网卡信息一致

**更新说明**：
本次大版本升级主要解决了用户反馈的网卡型号显示问题，新增了完整的无线网卡品牌型号检测功能，并彻底修复了中文乱码问题。现在工具能够智能识别腾达等USB网卡品牌，以及联想等笔记本内置网卡品牌，提供更准确的设备信息。
